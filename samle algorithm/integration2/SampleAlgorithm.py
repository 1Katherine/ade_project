...
class Samplealgorithm():
    def __init__(self, pbounds):
        self._samplespace = SampleSpace(pbounds)
        self._dim = self._samplespace._dim
        self._bounds = self._samplespace._bounds
        self._keys = self._samplespace._keys
    ...
    def LHSample(self, D, bounds, N):
        ...
        print('lhs标准差为 ： ' + str(np.std(np.asarray(result))))
        return result

    def RandomSample(self, D, bounds, N):
        '''
        :param D:参数个数
        ...
        print('random标准差为 ： ' + str(np.std(np.asarray(result))))
        return result

    # 将第一坐标和第二坐标画图
    def draw_sample(self, D, bounds, keys,  N, kind):
        # 根据lhs间隔，计算范围的间隔距离
        ...
        if kind.lower() == 'random':
            samples = self.RandomSample(D,bounds,N)
        elif kind.lower() == 'lhs':
    ...
    # 获得抽样样本
    def res(self, D, bounds, keys, N, kind):
        # 使用LHS生成样本(返回的所有样本点嵌套在一个大列表里）
        if kind.lower() == 'random':
            samples = self.RandomSample(D, bounds,N)
        elif kind.lower() == 'lhs':
    ...
    def output_sanples(self, samples):
        for i in samples:
            # print(i)
            pass
